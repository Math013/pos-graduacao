...
## FUNÇÃO PARA PLOTAR GRÁFICOS DE ALGEBRA LINEAR
...
# Cada linha é uma seta: de (x, y) até (xend, yend)
df <- data.frame(
  x    = c(0, 0, 0, proj[1]),
  y    = c(0, 0, 0, proj[2]),
  xend = c(x[1], y[1]*4, proj[1], x[1]),     # y escalado pra aparecer bem
  yend = c(x[2], y[2]*4, proj[2], x[2]),
  nome = c("x = (3,4)", "y = (1,0)", "proj(x→y) = (3,0)", "resíduo = (0,4)"),
  cor  = c("tomato", "steelblue", "darkgreen", "purple")
)

# === Quadradinho de ortogonalidade ===
# Fica no ponto onde a projeção termina (3, 0)
quad_size <- 0.2
quadrado <- data.frame(
